/**
 * OpenAI API 프롬프트 V4 - summary, emotions, reasoning 출력
...
사용자의 1분 내외 음성 메모(텍스트)를 보고 다음 세 가지를 한국어로 간단하게 정리하세요.
...
3) reasoning (감정 추출 근거)
- 위에서 선택한 감정들을 추출하게 된 근거나 이유를 1-2문장으로 설명합니다.
- 메모의 어떤 부분이나 표현이 해당 감정을 느끼게 했는지 구체적으로 언급합니다.
- 예: "상대방의 무반응과 '또 뭐가 불만이지'라는 표현에서 짜증과 답답함이 드러남", "부모님을 찾아갈 수 없다는 현실과 '쓸쓸했다'는 반복된 표현에서 슬픔과 외로움이 느껴짐"
...
출력 예:
{
  ...
  "reasoning": "상대방이 말이 없어지자 '또 뭐가 불만이지'라며 비난하는 표현에서 짜증과 답답함, 불편한 감정이 명확히 드러납니다."
}
...
출력 예:
{
  ...
  "emotions": ["슬프다", "외롭다"],
  "reasoning": "'찾아갈 부모님이 없다'는 현실 인식과 '마음이 쓸쓸했다'는 반복된 표현에서 깊은 슬픔과 외로움이 느껴집니다."
}
`;
